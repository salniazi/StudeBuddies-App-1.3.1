...
- (void)viewDidLoad
{
    ...
    if (self){
        ...
        [self.navigationController.navigationBar setBarTintColor:[UIColor colorWithRed:(21.0/255.0) green:(113.0/255.0) blue:(184.0/255.0) alpha:1.0]];
...
- (UIView*)  checkHit:(CGPoint)point {
    ...
    for (int i =0; i < PointsArray.count; i++) {
        ControlPointView *cpv = PointsArray[i];
        
        float center1= [cpv center].x;
        float center2 = [cpv center].y;
        
        if (sqrt(pow((point.x-view.center.x),2) + pow((point.y-view.center.y),2))>sqrt(pow((point.x-center1),2) + pow((point.y- center2),2)))
            view = PointsArray[i];
    }
