...
- (UITableViewCell *)tableView:(UITableView *)tableView cellForRowAtIndexPath:(NSIndexPath *)indexPath
{
    ...
    NSDictionary *dataForRowAtIndexPath ;
    int count = [[self applyFilterWithSearchQuery:self.text] count];
     if (count == 0)
     {
         dataForRowAtIndexPath=[data objectAtIndex:indexPath.row];
     }
    else
    {
        dataForRowAtIndexPath = [[self applyFilterWithSearchQuery:self.text] objectAtIndex:indexPath.row];
    }
